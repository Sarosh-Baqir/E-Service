...
const createService = async (req, res) => {
  try {
    ...
    const isServiceAvailable = await database.query.service.findFirst({
      where: and(eq(service.user_id, req.loggedInUserId), eq(service.category_id, category_id)),
    })
    if (isServiceAvailable) return errorResponse(res, "Not Allowed! You already created a service against this category.", 400)
...
const updateService = async (req, res) => {
  try {
    ...
    if (!isServiceAvailable) return errorResponse(res, "This service does not exist.", 400)
...
const deleteService = async (req, res) => {
  try {
    ...
    const data = await database.query.service.findFirst({ where: eq(service.id, id) })
    if (!data) {
...
const uploadServiceCoverPhoto = async (req, res) => {
  try {
    ...
    const updatedService = await database.update(service).set({ cover_photo: coverPhotoPath }).where(eq(service.id, service_id)).returning()
...
const fetchFilteredServices = async (req, res) => {
  try {
    ...
    if (category_id && city) {
      if (PLTH) {
        const services = await database.query.service.findMany({
          where: and(eq(service.city, city), eq(service.category_id, category_id)),
          with: {
            user: {
              columns: {
                first_name: true,
