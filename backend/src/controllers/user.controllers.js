...
const verifyUser = async (req, res) => {
  try {
    ...
    try {
      ...
      const updatedUser = await database.update(user).set({ is_verified: true, otp: null }).where(eq(user.email, email)).returning({
        id: user.id,
...
const switchRole = async (req, res) => {
  try {
    ...
    const data = await database
      ...
      .where(eq(user.id, req.loggedInUserId))
      .returning()
    const userData2 = await database.select().from(user).where(eq(user.id, req.loggedInUserId)).leftJoin(role, eq(role.id, user.role_id))
...
const getFriend = async (req, res) => {
  try {
    ...
    console.log(user_id)
    const data = await database.query.user.findFirst({ where: eq(user.id, user_id) })
    if (!data) {
      return successResponse(res, "No data Found against this user", data)
    }
    return successResponse(res, "User data is fetched successfully!", data)
